...
func setExamples(use string) string {
	examples := `  myc set ` + use + ` home <id> description "updated from cli"
  myc set ` + use + ` home <id> data.onConfig --file onConfig.js`
	if use == "field" {
		examples = `  myc set field home mysensor.1.dht.temperature formatter.onReceive --file on_receive.js
  myc set field home mysensor.1.dht.temperature formatter.onReceive "return value;"
  myc set field home <id> --path formatter.onReceive --file on_receive.js
  myc set field home gw1.1.1.V_CUSTOM name "Custom"`
	}
	if use == "gateway" {
		examples = `  myc set gateway home mysensor description "USB gateway"
  myc set gateway home mysensor provider.protocol.script --file script.js`
	}
	if use == "node" {
		examples = `  myc set node home mysensor.1 name "Living Room"
  myc set node home <id> others.note --file note.txt`
	}
	if use == "source" {
		examples = `  myc set source home mysensor.1.dht name "DHT"
  myc set source home <id> others.script --file script.js`
	}
	if use == "firmware" {
		examples = `  myc set firmware home stm32-app-slot-a description "slot A"
  myc set firmware home stm32-app-slot-a labels.ms_flash_slot A`
	}
	if use == "data-repository" {
		examples = `  myc set data-repository home ota_stm32_ab data.onConfig --file onConfig.js
  myc set data-repo home ota_stm32_ab data.onBlock --file onBlock.js`
	}
	return examples
}
