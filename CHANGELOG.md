...
- Indic console locales (Hindi, Kannada, Malayalam, Tamil, Telugu): IoT resource names (gateway, node, source, field, firmware, data repository, resource, virtual device, virtual assistant, payload, forward payload, quick ID, protocol, label, library) are native-script loanwords (Tamil கேட்வே, லேபிள், லைப்ரரி), not Latin English and not calques such as நுழைவாயில். Product names and protocol abbreviations stay Latin.
