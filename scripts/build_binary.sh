...
if [[ "${TARGET_BINARY}" == "core" ]]; then  # build core binary
  GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -v -tags=core -o mycontroller-core -ldflags "$LD_FLAGS" cmd/core/main.go
  ...
  GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -v -tags=core -o mycontroller-all-in-one -ldflags "$LD_FLAGS" cmd/all-in-one/main.go
